...
def add_args(parser):
    ...
    parser.add_argument("--iterations", help="number of iteration, including warm up", default=0, type=int)
...
def parse_all(events, ops, cnt, dur):
    ...
    for i in range(len(events)):
        if events[i]['name'] in ops:
            name = events[i]['name']
            if events[i]['ph'] == 'B' and events[i+1]['name'] == name and events[i+1]['ph'] == 'E':
                cnt[str(name)] += 1
...
def print_all(cnt, dur, iters=0):
    ...
    for i in range(len(cnt)):
        ...
        if iters != 0:
            ...
            str3 = ('%%-%ds ms/call' % maxpercall) % (dur[name] / 1000.0 / cnt[name])
...
def init_table(events):
    ...
    dur = {}
    for i in range(len(events)):
        if events[i]['name'] != 'process_name' and events[i]['name'] not in ops:
